...
from hummingbot.data_feed.candles_feed.data_types import (
    CandlesConfig,
...
class BacktestingDataProvider(MarketDataProvider):
    ...
    async def get_candles_feed(self, config: CandlesConfig):
        ...
        if not existing_feed.empty:
            ...
            if (
                existing_feed_start_time <= self.start_time
                and existing_feed_end_time >= self.end_time
            ):
                return existing_feed
        # Create a new feed or restart the existing one with updated max_records
        candle_feed = CandlesFactory.get_candle(config)
        candles_buffer = (
            config.max_records * CandlesBase.interval_to_seconds[config.interval]
        )
    ...
    def get_candles_df(
        ...
        """
        candles_df = self.candles_feeds.get(
    ...
    def get_price_by_type(
        self, connector_name: str, trading_pair: str, price_type: PriceType
    ):
        """
    ...
    def quantize_order_amount(
        self, connector_name: str, trading_pair: str, amount: Decimal
    ):
        """
    ...
    def quantize_order_price(
        self, connector_name: str, trading_pair: str, price: Decimal
    ):
        """
