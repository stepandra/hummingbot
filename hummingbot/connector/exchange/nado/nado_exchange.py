...
from hummingbot.connector.exchange.nado.nado_api_user_stream_data_source import (
    NadoAPIUserStreamDataSource,
)
...
class NadoExchange(ExchangePyBase):
    web_utils = web_utils

    def __init__(
        self,
        nado_ink_address: str,
        nado_ink_private_key: str,
        ...
        self.sender_address = utils.convert_address_to_sender(nado_ink_address)
        self.private_key = nado_ink_private_key
        self._use_spot_leverage = False
        # NOTE: Nado doesn't submit all balance updates, instead it only updates the product on position change (not cancel)
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return NadoAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return NadoAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            api_factory=self._web_assistants_factory,
            connector=self,
        ...
        **kwargs,
        ...
        try:
            ...
            order_result = await self._api_post(
                path_url=CONSTANTS.POST_PATH_URL, data=place_order, limit_id=limit_id
            )
            ...
            if matches_data is not None:
                for trade in matches_data:
                    ...
                    fee = TradeFeeBase.new_spot_fee(
                        ...
                        if Decimal(trade_amount) < s_decimal_0
                        else TradeType.BUY,
                        flat_fees=[
                            TokenAmount(
    ...
    async def _update_balances(self):
        ...
        for spot_balance in account["spot_balances"]:
            try:
                ...
                if (
                    product_id not in self._exchange_market_info[self._domain]
                    and product_id != 0
                ):
                    continue

                asset_name = self._exchange_market_info[self._domain][product_id][
                    "symbol"
                ]
        ...
        for product in exchange_info["data"]["spot_products"]:
            if product["product_id"] in symbol_data:
                try:
                    ...
                    product.update(
                        {"market": f"{symbol_data[product_id]}/{CONSTANTS.QUOTE}"}
                    )
                    product.update(
                        {
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for product_id in filter(utils.is_exchange_information_valid, exchange_info):
            ...
            # NOTE: Quote asset doesn't have a "market"
            if product_id == 0:
                continue
    ...
    async def _get_account_max_withdrawable(self):
        ...
        if (
            ...
            raise IOError(
                f"Unable to get available balance of product {0} for {sender_address}"
            )

        available_balances.update(
            {0: Decimal(utils.convert_from_x18(response["data"]["max_withdrawable"]))}
        )
        ...
        for trading_pair in trading_pairs:
            ...
            if (
                ...
                raise IOError(
                    f"Unable to get available balance of product {product_id} for {sender_address}"
                )
