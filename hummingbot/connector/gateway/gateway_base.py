...
class GatewayBase(ConnectorBase):
    ...
    def __init__(
        ...
        self._tokens = set()
        [
            self._tokens.update(set(trading_pair.split("_")[0].split("-")))
        ...
        self._order_tracker: ClientOrderTracker = ClientOrderTracker(
            connector=self, lost_order_count_limit=10
        )
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        """
        try:
            tokens = await GatewayHttpClient.get_instance().get_tokens(
                self._chain, self._network
            )
    ...
    def limit_orders(self) -> List[LimitOrder]:
        return [
            in_flight_order.to_limit_order() for in_flight_order in self.gateway_orders
        ]
    ...
    async def start_network(self):
        ...
        if not self._chain or not self._network:
            (
                ...
                network,
                error,
            ) = await self._get_gateway_instance().get_connector_chain_network(
        ...
        if not self._wallet_address:
            (
                wallet_address,
                error,
            ) = await self._get_gateway_instance().get_default_wallet(self._chain)
            if error:
    ...
    async def get_chain_info(self):
        ...
        try:
            ...
            self.logger().debug(
                f"Getting native currency for chain={self.chain}, network={self.network}"
            )
            native_currency = (
                await self._get_gateway_instance().get_native_currency_symbol(
                    chain=self.chain, network=self.network
                )
            )
            if native_currency:
                ...
                self.logger().info(
                    f"Set native currency to: {self._native_currency} for {self.chain}-{self.network}"
                )
            else:
                self.logger().error(
        ...
        try:
            ...
            if fee is not None and fee_asset is not None:
                ...
                self.network_transaction_fee = TokenAmount(
                    token=fee_asset, amount=Decimal(str(fee))
                )
    ...
    def ready(self):
        ...
        if not all(status.values()):
            ...
            self.logger().debug(
                f"Connector {self.name} not ready. Missing: {not_ready}. Status: {status}"
            )
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        status = {
            ...
            "network_transaction_fee": has_network_fee
            if self._trading_required
            else True,
        }
    ...
    async def update_balances(self, on_interval: bool = False):
        ...
        if not on_interval or (current_tick - last_tick) > self.UPDATE_BALANCE_INTERVAL:
            ...
            self._in_flight_orders_snapshot = {
                k: copy.copy(v) for k, v in self._order_tracker.all_orders.items()
            }
    ...
    def _handle_operation_failure(
        self, order_id: str, trading_pair: str, operation_name: str, error: Exception
    ):
        """
    ...
    async def update_order_status(self, tracked_orders: List[GatewayInFlightOrder]):
        ...
        for tracked_order, tx_details in zip(tracked_orders, update_results):
            ...
            if tx_status == TransactionStatus.CONFIRMED.value:
                self.process_transaction_confirmation_update(
                    tracked_order=tracked_order, fee=Decimal(str(fee or 0))
                )
    ...
    def process_transaction_confirmation_update(
        self, tracked_order: GatewayInFlightOrder, fee: Decimal
    ):
        fee_asset = (
    ...
    def update_order_from_hash(
        ...
        fee = 0
        if "data" in transaction_result and isinstance(
            transaction_result["data"], dict
        ):
    ...
    async def _monitor_transaction_status(
        self, order_id: str, transaction_hash: str, check_interval: float = 2.0
    ):
        """
        ...
        if not tracked_order:
            self.logger().warning(
                f"Order {order_id} not found in tracker, cannot monitor transaction status"
            )
            return
        ...
        while attempts < max_attempts and tracked_order.current_state not in [
            OrderState.FILLED,
            OrderState.FAILED,
            OrderState.CANCELED,
        ]:
            try:
                ...
                if tx_status == TransactionStatus.CONFIRMED.value:
                    ...
                    self.logger().info(
                        f"Transaction {transaction_hash} confirmed for order {order_id}"
                    )
                    break
                ...
                # Transaction failed
                    ...
                    self.logger().error(
                        f"Transaction {transaction_hash} failed for order {order_id}"
                    )
                    order_update = OrderUpdate(
                        client_order_id=order_id,
                        trading_pair=tracked_order.trading_pair,
                ...
                self.logger().error(
                    f"Error monitoring transaction status for {order_id}: {str(e)}",
                    exc_info=True,
                )
                break
        ...
        if attempts >= max_attempts:
            self.logger().warning(
                f"Transaction monitoring timed out for order {order_id}, transaction {transaction_hash}"
            )
    ...
    async def _update_single_balance(self, currency: str):
        ...
        try:
            ...
            self.logger().error(
                f"Error updating balance for {currency}: {str(e)}", exc_info=True
            )
    ...
    async def approve_token(
        ...
        try:
            ...
            self.logger().info(
                f"Token approval submitted. Order ID: {order_id}, Transaction: {transaction_hash}"
            )
            ...
            self.logger().error(
                f"Error approving {token_symbol}: {str(e)}", exc_info=True
            )
            raise
