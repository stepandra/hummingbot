...
from hummingbot.connector.exchange.nado import nado_utils


class NadoUtilTestCases(TestCase):
    ...
    def test_hex_to_bytes32(self):
        ...
        self.assertEqual(expected_bytes, nado_utils.hex_to_bytes32(hex_string))

    def test_convert_timestamp(self):
        timestamp = 1685989014506281744
        expected_ts = 1685989014506281744 / 1e9
        self.assertEqual(expected_ts, nado_utils.convert_timestamp(timestamp))

    def test_trading_pair_to_product_id(self):
        trading_pair = "WBTC-USDT0"
        expected_id = 1
        exchange_info = self.get_exchange_market_info_mock()
        self.assertEqual(
            expected_id,
            nado_utils.trading_pair_to_product_id(trading_pair, exchange_info),
        )
        missing_trading_pair = "ABC-XYZ"
        expected_missing_id = -1
        self.assertEqual(
            expected_missing_id,
            nado_utils.trading_pair_to_product_id(missing_trading_pair, exchange_info),
        )

    def test_market_to_trading_pair(self):
        market = "WBTC/USDT0"
        expected_trading_pair = "WBTC-USDT0"
        self.assertEqual(
            expected_trading_pair, nado_utils.market_to_trading_pair(market)
        )

    def test_convert_from_x18(self):
        data_numeric = 26369000000000000000000
        expected_numeric = "26369"
        self.assertEqual(expected_numeric, nado_utils.convert_from_x18(data_numeric))
        ...
        self.assertEqual(expected_dict, nado_utils.convert_from_x18(data_dict))

    def test_convert_to_x18(self):
        data_numeric = 26369.123
        expected_numeric = "26369000000000000000000"
        self.assertEqual(
            expected_numeric, nado_utils.convert_to_x18(data_numeric, Decimal("1"))
        )
        ...
        self.assertEqual(expected_dict, nado_utils.convert_to_x18(data_dict))

    def test_generate_expiration(self):
        timestamp = 1685989011.1215873
        expected_expiration = "1686075411"
        self.assertEqual(expected_expiration, nado_utils.generate_expiration(timestamp))

    def test_build_order_appendix(self):
        self.assertEqual(
            1, nado_utils.build_order_appendix(CONSTANTS.TIME_IN_FORCE_GTC)
        )
        self.assertEqual(
            1537, nado_utils.build_order_appendix(CONSTANTS.TIME_IN_FORCE_POSTONLY)
        )
    ...
    def test_generate_nonce(self):
        ...
        random.seed(42)
        self.assertEqual(
            expected_nonce, nado_utils.generate_nonce(timestamp, expiry_ms)
        )

    def test_convert_address_to_sender(self):
        address = "0xbbee07b3e8121227afcfe1e2b82772246226128e"  # noqa: mock
        expected_sender = (
            "0xbbee07b3e8121227afcfe1e2b82772246226128e64656661756c740000000000"  # noqa: mock
        )
        self.assertEqual(expected_sender, nado_utils.convert_address_to_sender(address))

    def test_is_exchange_information_valid(self):
        self.assertTrue(nado_utils.is_exchange_information_valid({}))
