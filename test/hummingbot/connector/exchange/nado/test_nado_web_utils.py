...
class WebUtilsTests(TestCase):
    def test_public_rest_url(self):
        url = web_utils.public_rest_url(
            path_url=CONSTANTS.QUERY_PATH_URL, domain=CONSTANTS.DEFAULT_DOMAIN
        )
        self.assertEqual("https://gateway.prod.nado.xyz/v1/query", url)
        url = web_utils.public_rest_url(
            path_url=CONSTANTS.QUERY_PATH_URL, domain=CONSTANTS.TESTNET_DOMAIN
        )
        self.assertEqual("https://gateway.test.nado.xyz/v1/query", url)
    ...
    def test_get_current_server_time(self):
        ...
        server_timestamp = loop.run_until_complete(
            asyncio.wait_for(
                web_utils.get_current_server_time(web_utils.create_throttler()), 1
            )
        )
